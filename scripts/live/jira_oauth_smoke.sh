#!/usr/bin/env bash
# Opt-in smoke script for Jira Phase 1 browser OAuth + read-only tool call.
...
start_url="${MCP_PUBLIC_BASE_URL}/auth/jira/start?code=${MCP_LINK_CODE}"
...
###############################################################################
# Prompt for a safe read-only Jira tool
###############################################################################
default_tool='jira_get_all_projects'
read -rp "Enter read-only Jira tool to invoke [${default_tool}]: " tool_name
tool_name="${tool_name:-$default_tool}"

read -rp 'Enter JSON args (or leave blank for {}): ' args_json
args_json="${args_json:-{}}"
